...
int main(int argc, char** argv)
{
    ...
    if(argc > 1) {
        for(int i = 1; i < argc; i++) {
            ...
            if(string(argv[i]) == "-k" && argc > i+1) kernelSize = atoi(argv[i+1]);
            if(string(argv[i]) == "-f" && argc > i+1) in = argv[i+1];
        }
    }
    ...
    cout << "  Kernel Size: " << kernelSize << endl << endl;
    ...
    if(testmode) dout = bufferTest(0,width,height,kernelSize,dout);
    else bufferTest(0,width,height,kernelSize,dout);
